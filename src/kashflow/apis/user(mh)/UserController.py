...
class User(Resource):
    ...
    def post(self):
        ...
        token, key = create_token(str(newUserObjectId))
        tokenObj = {
        ...
        emailResponse = send_email('mightyhost0@gmail.com', args['email'], 'Verify email address',
                                  'http://localhost:5000/user/verify?token=' + verificationToken)
...
class Login(Resource):
    ...
    def post(self):
        ...
        if bcrypt.checkpw(args['password'].encode('utf8'), userData['password']):
            ...
            token, key = create_token(str(userData['_id']))
            tokenObj = {
